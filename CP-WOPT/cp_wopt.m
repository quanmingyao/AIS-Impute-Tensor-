function [P, output] = cp_wopt(Z, W, R, para, testD)
...
if(isfield(para, 'tol'))
    options.RelFuncTol = para.tol;
else
    options.RelFuncTol = 1e-3;
end
...
clear obj timeFlag Time testData;

global obj timeFlag Time RMSE;
obj = [];
Time = [];
RMSE = [];
timeFlag = tic;
...
while(length(obj) < 200)
    out = feval(opthandle, funhandle, tt_fac_to_vec(P0), options);
    P0 = tt_cp_vec_to_fac(out.X,Z);
end

% out = ncg(funhandle, tt_fac_to_vec(P0), 'Update', 'PR', 'Display', 'iter', 'StopTol', 1e-10, 'RelFuncTol', 1e-5, 'MaxIters', 5000);
...
global obj Time timeFlag RMSE testData;
obj = [obj, f];
Time = [Time, toc(timeFlag)];

if( ~isempty(testData) )
    RMSE = [RMSE, CWOPTRMSE( A, testData )];
